...
cap cd "C:/Users/lmostrom/Dropbox/Amitabh/PubMed/PMIDs"

*-------------------------------------------------------------------------------
*First, QA list
if `qa_prep' == 1 {
*-------------------------------------------------------------------------------
local filelist: dir "QA" files "PMIDs_all_TotalQA_????.csv"

local i = 1
foreach file of local filelist {
	import delimited pmid using "QA/`file'", rowr(2:) clear
	dis "`file'"
	if _N > 0 {
...
if `notqa_prep' == 1 {
*-------------------------------------------------------------------------------
import delimited pmid using "PMIDs_master_2019.csv", clear rowr(2:)
tempfile pmids2019
save `pmids2019', replace

import delimited pmid using "PMIDs_master_samp5pct.csv", clear rowr(2:)
...
foreach QA_not in "QA" "notQA" {
	...
	isid pmid
	save "../Master_dta/pmids_`QA_not'_wos_funding.dta", replace
}
