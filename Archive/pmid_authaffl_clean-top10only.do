...
if `pmids_append_2005' == 1 {
*---------------------------
local filelist: dir "PMIDs\2005-2018\" files "*.csv"

local i = 1
foreach file of local filelist {
	import delimited pmid query_name using "PMIDs/2005-2018/`file'", rowr(2:) clear
	dis "`file'"
	if `i' == 1 {
		tempfile full_pmids
		save `full_pmids', replace 
...
gen year = substr(query_name, -4, 4)
	destring year, replace
gen nih = substr(query_name, -8, 4) == "_NIH"
gen pub = inlist(substr(query_name, -8, 4), "_NIH", "_Pub")
gen lifesci = substr(query_name, 1, 7) == "LifeSci"
...
save pmids_QAqueries_full_2005.dta, replace
...
if `affls_clean' == 1 {
*---------------------------
include "$repo/clean_msa_codes2-top10only.do"
...
use "full_auth_affls`ext'.dta", clear
	...
	*Then search strings for state names
	...
	foreach sn of local state_names {
		...
		replace state_name = "`sn'" if strpos(affl, "`sn'") > 0 & state_name != "" & ///
										strpos(affl, "`sn'") < strpos(affl, state_name)
	}
	...
	*Then search strings for state abbreviations
	...
	foreach SA of local state_abbrs {
		...
		replace state_abbr = "`SA'" if strpos(affl, "`SA'") > 0 & state_abbr == ""
		replace state_abbr = "`SA'" if state_abbr != "" & strpos(affl, "`SA'") > 0 & ///
										strpos(affl, "`SA'") < strpos(affl, state_abbr)
...
merge 1:1 pmid using "pmids_QAqueries_full_2005.dta", nogen keep(2 3)

save affls_master_2005.dta, replace
*---------------------------
}
*---------------------------
